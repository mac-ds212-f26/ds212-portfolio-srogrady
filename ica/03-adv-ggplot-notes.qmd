...
> The range helps identify how far out of range the temperatures were, and the color red catches a viewers eye. 

In particular, based on the intro paragraph,
  - What key message/claim does NYTimes want readers to be able to explore? 
  
> How the temps changed over the months in the city, and how it differs for city. 

  - How did this goal inform what information is displayed in the visualization?
  
> The x and y axis's choosen and the key pointed out record high temps with arrows. 

**Aesthetic Mapping** 

- What specific variables (from the data codebook) underlie the visualization?

> Month, RecordLow/RecordHigh,Precip

- How do these variables map to aesthetics of the visual elements, eg, position, size, shape, and color of glyphs?

> The month is the X axis and it is labeled jan-Dec, Record lows and high have a key and are visual layers on the graph set behind the actual observed temps. 
...
ggplot(weather) +
  geom_linerange(aes(x = dateInYear, ymin = RecordLow, ymax = RecordHigh), color = '#ECEBE3') +
  geom_linerange(aes(x = dateInYear, ymin = NormalLow, ymax = NormalHigh), color = "#C8B8BA")+ geom_linerange(aes(x = dateInYear, ymin = Low, ymax = High), color = "#A90248")+ 
    theme_classic()
```
...
month_weather <- weather |>
  group_by(Month)|>
  summarize(dateline = min(dateInYear))
```
...
ggplot() +
  geom_linerange(aes(x = dateInYear, ymin = RecordLow, ymax = RecordHigh), color = '#ECEBE3') +
  geom_linerange(aes(x = dateInYear, ymin = NormalLow, ymax = NormalHigh), color = "#C8B8BA")+ geom_linerange(aes(x = dateInYear, ymin = Low, ymax = High), color = "#A90248") + 
  geom_vline(data = month_weather, aes(xintercept = dateline),color = '#ECEBE3') +
    theme_classic()
```
...
ggplot() +
  ...
  geom_vline(data = month_weather, aes(xintercept = dateline),color = '#ECEBE3') +
  scale_x_continuous(
    breaks = month_weather$dateline,
    labels = month_weather$month
  )+
    theme_classic()
